...
function snapshotAllPanels(queryClient: QueryClient): SnapshotEntry[] {
  return [
    ...queryClient.getQueriesData({ queryKey: ['left'] }),
    ...queryClient.getQueriesData({ queryKey: ['right'] }),
  ];
}

function rollbackPanels(
...
function applyQueuedSnapshot(
  queryClient: QueryClient,
  ack: Pick<StateMeta, 'stateVersion' | 'pendingAdd' | 'pendingMutation'>
) {
...
export function useActions() {
  ...
  const mutationResyncTimer = useRef<ReturnType<typeof setTimeout> | undefined>(
    undefined
  );
  const addResyncTimer = useRef<ReturnType<typeof setTimeout> | undefined>(
    undefined
  );

  const scheduleMutationListsResync = () => {
    if (mutationResyncTimer.current !== undefined)
      clearTimeout(mutationResyncTimer.current);
  ...
  const scheduleAddResync = () => {
    if (addResyncTimer.current !== undefined)
      clearTimeout(addResyncTimer.current);
  ...
  const selectMutation = useMutation({
    ...
    onError: (_error, _id, context) =>
      rollbackPanels(queryClient, context?.snapshots),
  });
  ...
  const reorderMutation = useMutation({
    ...
    onMutate: async ({ itemId, targetId, position }) => {
      ...
      queryClient.setQueriesData({ queryKey: ['right'] }, (oldData) =>
        reorderLoadedRight(
          oldData as InfiniteData<CursorPage, number | null>,
          itemId,
          targetId,
          position
        )
